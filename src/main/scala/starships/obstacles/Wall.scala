...
import starships.obstacles.Wall._
...
case class Wall(
  ...
  def draw(p: PApplet): Unit = {
    ...
    if (location.y == 0) {
      ...
      p.image(WallRootBottom, location.x, location.y, 80, 20)
      ...
      p.image(WallRootTop, location.x, location.y + size.y - 20, 80, 20)
...
object Wall {
  ...
  def loadImages(p: PApplet): Unit = {
    ...
    WallRootBottom = p.loadImage("src/main/Resources/WallRootBottom.png")
